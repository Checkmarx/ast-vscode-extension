...
export async function multiStepInput(logs: Logs, context: vscode.ExtensionContext) {
  ...
  update(context, PROJECT_ID_KEY, {id: state.project.id, name: `${PROJECT_LABEL} ${state.project.label}`});
  update(context, BRANCH_ID_KEY, {id: state.branch.id, name: `${BRANCH_LABEL} ${state.branch.label}`});
  update(context, SCAN_ID_KEY, {id: state.scanId.id, name: `${SCAN_LABEL} ${state.scanId.label}`});
