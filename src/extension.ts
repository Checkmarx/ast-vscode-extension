...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      `${EXTENSION_NAME}.kicsRealtime`,
      async () => await kicsProvider.runKics()
    )
  );
  ...
  astResultsProvider
    .openRefreshData()
    .then(() =>
      logs.info("Data refreshed and synced with Checkmarx One platform")
    );
  // Initialize filters state
  initializeFilters(logs, context, astResultsProvider).then(() =>
    logs.info("Filters initialized")
  );
  ...
  const scaTree = vscode.window.createTreeView("scaAutoScan", {
  ...
  // Settings
  context.subscriptions.push(
    vscode.commands.registerCommand(`${EXTENSION_NAME}.viewSettings`, () => {
      vscode.commands.executeCommand(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      `${EXTENSION_NAME}.viewKicsSaveSettings`,
      () => {
        vscode.commands.executeCommand(
          "workbench.action.openSettings",
          `Checkmarx KICS`
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(`${EXTENSION_NAME}.scanInput`, async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(`${EXTENSION_NAME}.showError`, () => {
      const err = getError(context);
      if (err) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      `${EXTENSION_NAME}.kicsRemediation`,
      async (
        fixedResults,
        kicsResults,
        file,
        ...
        await kicsProvider.kicsRemediation(
          fixedResults,
          kicsResults,
          file,
          diagnosticCollection,
