...
async function addRepositoryListener(
  ...
  return repoState.onDidChange(() => {
    ...
    updateState(context, constants.branchName, { id: branchName, name: branchName, formattedId: undefined, datetime: undefined});
    updateState(context, constants.branchTempIdKey, { id: branchName, name: branchName, formattedId: undefined, datetime: undefined}); //TODO: This is an hack to fix duplicated onchange calls when branch is changed.
...
export async function setScanButtonDefaultIfScanIsNotRunning(
  ...
  if (!scan?.id) {
    ...
    updateState(context, constants.scanCreatePrepKey, { id: false, name: "" , formattedId: "", datetime: ""});
  }
