...
export abstract class BaseScannerService implements IScannerService {
  ...
  protected getTempSubFolderPath(document: vscode.TextDocument, baseTempDir: string): string {
    const workspaceFolder = vscode.workspace.getWorkspaceFolder(document.uri)?.uri.fsPath || '';
    const relativePath = path.relative(workspaceFolder, document.uri.fsPath);
    return path.join(os.tmpdir(), baseTempDir, this.toSafeTempFileName(relativePath));
  }
