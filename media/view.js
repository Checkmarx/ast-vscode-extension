...
(function () {
  ...
  document.querySelectorAll(".package-next").forEach((element) => {
    element.addEventListener("click", (e) => {
      ...
      let currentPageLocations = document.getElementById(
        "locations-table-" + current
      );
      currentPageLocations.style.display = "none";
      let nextPageLocations = document.getElementById(
      ...
      counter.innerHTML =
        "<p>" + (parseInt(current, 10) + 1) + "/" + total + "</p>";
    });
  });
  ...
  document.querySelectorAll(".package-back").forEach((element) => {
    element.addEventListener("click", (e) => {
      ...
      nextPage.style.display = "";
      // Change the displayed tables for locations
      let currentPageLocation = document.getElementById(
        "locations-table-" + current
      );
  ...
  window.addEventListener("message", (event) => {
    ...
    switch (message.command) {
      ...
      case "loadLearnMore":
        ...
        let learnLoaderContainer = document.getElementById(
          "learn-container-loader"
        );
        learnLoaderContainer.style.display = "none";
        learnLoaderContainer.innerHTML = infoLearnContainer(
          learn,
          message.result
        );
    ...
    if (learnArray.length > 0) {
      for (let learn of learnArray) {
        for (let code of learn.samples) {
          ...
          codeTitlePara.textContent =
            "" + code.title + " using " + code.progLanguage;
  ...
  function codeBashingSection(result) {
    ...
    if (result.multipleSastNode.nodes.length > 0) {
      ...
      codeBashingSpan.setAttribute(
        ...
        "Learn more about " +
          result.queryName +
          " using Checkmarx's eLearning platform"
      );
  ...
  function infoChanges(change) {
    ...
    let severityClass =
      change.Severity.length > 0
        ? "select-" + change.Severity.toLowerCase()
        : "";
    ...
    var state =
      change.State.length > 0
        ? change.State.replaceAll("_", " ")
        : "No changes in state.";
  ...
  function loader() {
    ...
    historyContainerLoaderPara.setAttribute(
      "class",
      "history-container-loader"
    );
